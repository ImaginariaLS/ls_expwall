    ...
    function loadNext() {
        ...
        ls.ajax(url, params, function (result) {
            if (result.bStateError) {
                ...
                var iCount = parseInt($('#wall-count-next').text());
                if (result.iCountWall) {
                    $('#wall-container').append(result.sText);
                    ipage++;
                }
                iCount = iCount - result.iCountWallReturn;
                if (iCount) {
...
{if $oUserCurrent}
    <form class="wall-submit">
        <textarea rows="4" id="wall-text" class="mce-editor markitup-editor input-width-full js-wall-reply-parent-text"
